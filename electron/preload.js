...
contextBridge.exposeInMainWorld("kiln", {
  ...
  tagImage: (source, target) => ipcRenderer.invoke("kiln:tag-image", { source, target }),
  ...
  volumeSnapshots: (name) => ipcRenderer.invoke("kiln:volume-snapshots", name),
  createVolumeSnapshot: (name, keep) => ipcRenderer.invoke("kiln:create-volume-snapshot", { name, keep }),
