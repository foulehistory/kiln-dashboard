...
/** One independent terminal session: its own xterm instance, its own
 * exec session, its own connection dropdown. `TerminalView` mounts one
 * of these per open tab and keeps every tab mounted (just hidden) so
 * switching tabs never tears down a session mid-command. */
export default function TerminalTab({ visible }: { visible: boolean }) {
  ...
  useEffect(() => {
    ...
    // Ctrl+Shift+C copy, Ctrl+Shift+V paste, Ctrl+L clear, Ctrl+F search -
    // Ctrl+C/Ctrl+V alone stay reserved for SIGINT/normal shell input,
    // matching most terminal emulators' convention.
  ...
  // A hidden tab's container has zero size - fit() while hidden would
  // shrink the terminal to 0 rows/cols, so only re-fit once it's shown
  // again (e.g. after switching back from another tab).
  useEffect(() => {
  ...
  function doSearch(direction: "next" | "prev") {
    ...
    if (direction === "next") searchAddonRef.current?.findNext(searchTerm);
    else searchAddonRef.current?.findPrevious(searchTerm);
  }
  ...
  return (
    <div>
      <div className="toolbar">
        ...
        <button className="icon-btn" title="Search buffer (Ctrl+F)" onClick={() => setShowSearch((s) => !s)} style={{ marginLeft: "auto" }}>
          <SearchIcon />
        </button>
      </div>
      {showSearch && (
        <div className="terminal-search-bar">
          ...
          />
          <button onClick={() => doSearch("prev")}>Prev</button>
          <button onClick={() => doSearch("next")}>Next</button>
          <button className="icon-btn" title="Close search" onClick={() => setShowSearch(false)}>
            <CloseIcon />
          </button>
        </div>
      )}
